...
| 能力 | 当前实现 |
|------|----------|
| 日志捕获 | `logcat -v threadtime,uid`；按 Tag / Level 过滤，并将配置包名映射到运行中进程 PID 后比较 `parts[2]` |
...
| 压缩归档 | 无时间范围时按日期生成 ZIP；有时间范围时生成单 ZIP；输出至 `/data/xclogger/mobilelog`；范围命名依赖 `startTime`，仅传 `endTime` 可触发空值异常 |
| 加密与审计 | 提供 XOR 加密类；关键流程尝试写入 `A_OperationHistory.txt` |
...
| `control` | `PartialConfigMerger` | 合并旧配置与 patch；仅合并 8 个基础字段；buffer 按 512 对齐、最大 4096 |
...
| `recorder` | `SystemLogCatcher` | 运行/监控 logcat 子进程，执行 Tag / Level / PID 过滤及包名到运行中 PID 的动态映射；新增 isLogcatAlive() 供外部诊断子进程存活 |
| `recorder` | `FilterPipeline` | 定义扩展白黑名单过滤规则；当前没有接入采集循环 |
| `filemanager` | `FileManager` | 日志文件创建、写入、轮转、清理和历史文件维护 |
| `filemanager` | `FileCompressService` | 异步压缩、上传状态、结果广播、ZIP 清理 |
...
| `receiver` | `PackageEventManager` | 安装/卸载包时维护 PID 集合和包名前缀刷新；刷新由日志读取循环约每 10 秒触发 |
| `util` | `XcLoggerConfig` | Parcelable 配置：8 个基础字段与 5 个扩展过滤字段 |
...
`RemoteBindService` 将 start、stop、配置更新和压缩请求提交至 `CommandSerialExecutor`。`getConfiguration()` 仅读取 `ConfigLoader.current()` 缓存，尚未加载时可返回 `null`。压缩完成后，`FileCompressService` 发送 `CTRL_RESULT`，`RemoteBindService` 的内部接收器再转发为 `IXcLoggerListener` 回调。

### 5.4 包事件与 PID 缓存刷新

`PACKAGE_ADDED` 经 `PackageEventManager.handlePackageInstalled()` 重新扫描匹配包的运行中 PID；`PACKAGE_REMOVED` 会移除该包对应的 PID 映射。无论配置使用精确包名还是以 `.` 结尾的前缀，`readLogcatOutput()` 都约每 10 秒刷新一次 PID 缓存。`parseLogLine()` 按 `threadtime,uid` 格式将 `parts[2]` 解析为 PID 后参与比较。
...
Tag 配置会生成 `*:S`、`TagA:E`、`TagB:E` 等独立 logcat filter spec；`AndroidRuntime`、`DEBUG`、`libc` 以 `:V` 追加，并在 Java 侧绕过当前过滤。Level 优先级为 `f > e > w > i > d > v`。Package 过滤扫描 `ActivityManager.getRunningAppProcesses()`，精确匹配 `pkgList` 中的完整包名；以 `.` 结尾的配置执行前缀匹配，再将命中的进程 PID 汇总到 `filterPidSet`。配置为精确小写 `all` 时不限制；已配置包过滤但当前没有匹配的运行中 PID 时，除上述关键崩溃 Tag 外，日志均不通过 Package 条件。
...
### 6.3 FilterPipeline 扩展字段状态

`FilterPipeline` 已定义 UID / Level / Content 白名单和 Tag / UID / Level / Content 黑名单。`XcLoggerConfig`、默认 XML、广播 patch 和数据库读取路径包含：
...
但当前 `SystemLogCatcher.readLogcatOutput()` 未调用 `FilterPipeline.shouldAccept()`，`filterState` 和黑白名单集合未参与读取循环，因此扩展黑白名单字段仍未生效；基础 Tag / Level / PID 过滤已参与读取循环。

`XcLoggerDatabase.putConfig()` 当前只保存 8 个基础字段，上述 5 个扩展字段不会由 `saveConfig()` / `saveInitialConfig()` 持久化；数据库重新加载时会得到空字符串。
...
因此 APK 变体为 `commonDebug`、`commonRelease`、`p1416TPinelabsDebug`、`p1416TPinelabsRelease`。每个变体的输出文件名都被设置为 `XCLogger_v<versionName>_<yyyyMMddHHmm>.apk`；debug 与 release 当前使用相同命名规则。
...
`afterEvaluate` 中额外注册了兼容性测试任务别名，并使 `assembleCommonDebug` 依赖 `assembleP1416TPinelabsDebug`；在 Android Studio 或命令行选择/执行 `assembleCommonDebug` 时，会连带构建两个 debug flavor。
...
| 目标 | Gradle 任务 |
|------|-------------|
| 构建 common Debug APK，并连带构建 PineLabs Debug APK | `./gradlew assembleCommonDebug` |
...
`app` 的 `customer` flavor 决定 APK applicationId 和默认配置 XML。`common` 使用 `com.xcheng.xclogger` 与 `default_config.xml`；`p1416TPinelabs` 从 `keystore.gradle` 读取 applicationId、签名并使用 `p1416t_pinelabs_default_config.xml`。外部绑定和广播回调必须使用实际安装 flavor 的 applicationId。
